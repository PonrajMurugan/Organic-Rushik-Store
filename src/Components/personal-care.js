...
const allPersonalCare=[
    ...
    },
    {
        id:8,
        image:per8 , 
        name:'Ocean Fresh Body Lotion Cream 250ml',
...
const PersonalCareDetails = () => {
    ...
    return (
        <div className='per-car-bk'>
            <h1 className='pcd-title'>Personal Care</h1>
            <div className="product-grid container">
                {allPersonalCare.map((product) => (
                    <div className="product-cards" key={product.id}>
                        <img src={product.image} alt={product.name} />
                        <h2>{product.name}</h2>
                        <p>Price:  ₹ {product.price}</p>
                        <Link to={`/details/${product.id}`}>
                            <button className="view-details-button"  onClick={handleCardClick}>View Details</button>
                        </Link>
                    </div>
                ))}
            </div>

...
const Agriculture = () => {
    ...
    return (
        <div className='agri-bk'> 

  
        <div className='agri-container'>
            <h1 className='agri-title'>Agriculture Products</h1>
            <div className='agri-product-grid'>
                {allAgriculture.map((product) => (
                    <div className='agri-product-card' key={product.id}>
                        <img className='agri-product-image' src={product.image} alt={product.name} />
                        <h2 className='agri-product-name'>{product.name}</h2>
                        <p className='agri-product-price'>Price: ₹ {product.price}</p>
                        <Link to={`/agriDetails/${product.id}`}>
                            <button className='agri-view-details-button' onClick={handleCardClick}>View Details</button>
                        </Link>
                    </div>
                ))}
            </div>
